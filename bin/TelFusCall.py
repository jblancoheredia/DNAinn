'''
Script modified by blancoj@mskcc.org for MSKCC target-seq panels
Last updated on 04APR25
'''
...
def ComputeCoverage(coverage_out,tmp,sample,panel):
    ...
    if len(cov_files) > 0:
        ...
        # Compute coverage
        if panel == "IM7":
            Coverage = round(int(Read_length)*int(Total_reads)/4681560,2)
        elif panel == "IM6":
            Coverage = round(int(Read_length)*int(Total_reads)/1522367,2)
        elif panel == "IM5":
            Coverage = round(int(Read_length)*int(Total_reads)/1363306,2)
        elif panel == "IM3":
            Coverage = round(int(Read_length)*int(Total_reads)/1204666,2)
        elif panel == "Av1":
            Coverage = round(int(Read_length)*int(Total_reads)/388850,2)
        elif panel == "Av2":
            Coverage = round(int(Read_length)*int(Total_reads)/429064,2)
        else:
            print('Warning: Inappropiate MSKCC target panel!')
            print({panel})
        # Saving results
...
def summary_read(read,Genome):
    ...
    Endogenous_fusions = {'Hg19':{'chr2':[114355250,114365750],'chr4':[000000000,000000000],'chr9':[130911250,130922000]},
                          'Hg38':{'chr2':[113597750,113608250],'chr4':[190177250,190188000],'chr9':[128149000,128159750]},
                     'T2T-CHM13':{'chr2':[114022500,114033000],'chr4':[190177250,190188000],'chr9':[140356250,140366750]}}

    if (Chrom != None):
        Chrom2 = 'chr'+regex.sub("^chr",'',Chrom)
    else:
        Chrom2 = 'none'

    if (Chrom2 in Endogenous_fusions[Genome].keys()):  # Reduced in targeted data to 6 and to 1 when UMI
    ...
    Telomere_ends = {"Hg19":{"chr1": [249140621, float('inf')],    "chr10": [135424747, float('inf')],    "chr11": [134896516, float('inf')],    "chr12": [133741895, float('inf')],
                ...
                "T2T-CHM13":{"chr1": [248846422, float('inf')],    "chr10": [133687422, float('inf')],    "chr11": [134976622, float('inf')],    "chr12": [133165309, float('inf')],
                            "chr13": [114254328, float('inf')],    "chr14": [106933718, float('inf')],    "chr15": [101881189, float('inf')],    "chr16": [90228345,  float('inf')],
                            "chr17": [83147441,  float('inf')],    "chr18": [80263285,  float('inf')],    "chr19": [58507616,  float('inf')],     "chr2": [242083529, float('inf')], 
                            "chr20": [64334167,  float('inf')],    "chr21": [46599983,  float('inf')],    "chr22": [50708468,  float('inf')],     "chr3": [198185559, float('inf')],
                             "chr4": [190104555, float('inf')],     "chr5": [181428259, float('inf')],     "chr6": [170695979, float('inf')],     "chr7": [159235973, float('inf')],
    ...
    if (int(MQ) >= 1 and Seq_length >= 50):
        if (Chrom2 in Telomere_ends[Genome].keys()):
            Region_end_chromosome = overlaps([Start,End],Telomere_ends[Genome][Chrom2])
            Region_start_chromosome = overlaps([Start,End],[0,10000000]) # We consider as telomere the first 100,000 bps of each chromosome
...
def GettingFilteringVariable(line,Duplicated_sequences,Duplicated_sequence_already_included):
    ...
    # FOR THE FUTURE ? --> Filter 8: Check if fusion is found in both reads
...
def initialize_parser():
    ...
    parser.add_argument('--panel', type=str, choices = ["IM3", "IM5", "IM6", "IM7", "Av1", "Av2"], default = 'Av2', help = 'MSKCC target panel used for the sequencing (Choose IM3 for IMPACT-341, IM5 for IMPAXCT-410, IM6 for IMPACT-468, IM7 for IMPACT-505, Av1 for ACCESSv1 or Av2 for ACCESSv2)', required=True)
...
def main():
    ...
    print (f"   MSKCC target panel: {panel}")
    ...
    Cov_flag = ComputeCoverage(coverage_out,tmp,sample,panel)
