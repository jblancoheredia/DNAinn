process STRELKA_SOMATIC {
    ...
    input:
    tuple val(meta),  path(input_tumour), path(input_index_tumour)
    tuple val(meta1), path(manta_candidate_small_indels)
    tuple val(meta2), path(manta_candidate_small_indels_tbi)
    path(input_normal)
    path(input_index_normal)
    path(target_bed)
