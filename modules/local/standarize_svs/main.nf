process STANDARDIZE_SVS {
    tag "$meta.id"
    label 'process_low'

    conda "conda-forge/label/cf202003::python"
    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        'docker://blancojmskcc/standardize_svs:1.3.0' :
        'blancojmskcc/standardize_svs:1.3.0' }"
    ...
    """
    python /usr/local/bin/standardize_svs.py ${delly_vcf}
    python /usr/local/bin/standardize_svs.py ${svaba_vcf}
    python /usr/local/bin/standardize_svs.py ${manta_vcf}
    python /usr/local/bin/standardize_svs.py ${tiddit_vcf}
    python /usr/local/bin/standardize_svs.py ${gridss_vcf}

    cat <<-END_VERSIONS > versions.yml
    "${task.process}":
        standardize_svs: "v1.3.0"
    END_VERSIONS
    """
    stub:
    def args = task.ext.args ?: ''
