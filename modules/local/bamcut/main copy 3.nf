process BAMCUT {
    ...
    """
    samtools view -@ $task.cpus -F 0x04 -u ${bam} | \\
    samtools sort -@ $task.cpus -n -O SAM - | \\
    awk -v n=1000000 -v FS="\\t" 'BEGIN {part=0; line=n} \\
        /^@SQ/ {sq_header = sq_header \$0 "\\n"; next;} \\
        /^@/ {other_header = other_header\$0"\\n"; next;} \\
        { \\
            if(line>=n && \$1!=last_read) { \\
                part++; \\
                line=1; \\
            } \\
            print line==1 ? sq_header other_header \$0 : \$0 | "samtools view -b -o ${prefix}_chunk_"part".bam"; \\
            close("samtools view -b -o ${prefix}_chunk_"part".bam"); \\
            last_read = \$1; \\
            line++; \\
        }'
