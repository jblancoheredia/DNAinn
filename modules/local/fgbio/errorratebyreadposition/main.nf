...
process FGBIO_ERRORRATEBYREADPOSITION_DUP {
    tag "$meta.id"
    label 'process_high'
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch  ${prefix}_dup.error_rate_by_read_position.txt
    touch  ${prefix}_dup.error_rate_by_read_position.pdf
...
process FGBIO_ERRORRATEBYREADPOSITION_SIM {
    tag "$meta.id"
    label 'process_high'
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch  ${prefix}_sim.error_rate_by_read_position.txt
    touch  ${prefix}_sim.error_rate_by_read_position.pdf
...
process FGBIO_ERRORRATEBYREADPOSITION_DR {
    tag "$meta.id"
    label 'process_high'
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch  ${prefix}_dr.error_rate_by_read_position.txt
    touch  ${prefix}_dr.error_rate_by_read_position.pdf
