process ALIGN_HLA_CHR6 {
    ...
    output:
    tuple val(meta), path("*.hla.chr6.mapped.bam"), emit: bam
    tuple val(meta), path("*.hla.chr6.mapped.bai"), emit: bai
    ...
    bwa-mem2 mem \\
        ...
        samtools calmd -@ ${task.cpus} -b ${prefix}_sorted.bam ${fasta} > ${prefix}.hla.chr6.mapped.bam && \\
        samtools index -@ ${task.cpus} ${prefix}.hla.chr6.mapped.bam ${prefix}.hla.chr6.mapped.bai
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch ${prefix}..hla.chr6.mapped.bam
    touch ${prefix}..hla.chr6.mapped.bai
