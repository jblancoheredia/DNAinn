...
process SURVIVOR_SCAN_READS_DUP {
    tag "$meta.id"
    label 'process_single'
    ...
    output:
    tuple val(meta), path("*.dup_error_profile.tsv"), emit: error_profile
    path "versions.yml"                             , emit: versions
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch ${prefix}.dup_error_profile.tsv
...
process SURVIVOR_SCAN_READS_SIM {
    tag "$meta.id"
    label 'process_single'
    ...
    output:
    tuple val(meta), path("*.sim_error_profile.tsv"), emit: error_profile
    path "versions.yml"                             , emit: versions
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch ${prefix}.sim_error_profile.tsv
...
process SURVIVOR_SCAN_READS_DR {
    tag "$meta.id"
    label 'process_single'
    ...
    output:
    tuple val(meta), path("*.dr_error_profile.tsv"), emit: error_profile
    path "versions.yml"                            , emit: versions
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch ${prefix}.dr_error_profile.tsv
